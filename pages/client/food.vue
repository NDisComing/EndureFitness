  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <h2 class="font-heading font-black text-xl uppercase tracking-tight text-brand-charcoal">Daily Meal Diary</h2>
      </div>

      <button
        @click="showCamera = true"
        class="bg-brand-accent text-white font-heading font-black text-xs uppercase tracking-wider px-3.5 py-2 rounded-xl hover:bg-brand-charcoal transition-all shadow-[0_4px_15px_rgba(250,129,18,0.35)] flex items-center gap-1.5 active:scale-95"
      >
    ...
    <div class="bg-brand-gray border border-brand-charcoal/10 rounded-2xl p-4 flex items-center justify-between gap-4 shadow-sm">
      <div>
        <p class="text-[10px] uppercase font-bold text-brand-charcoal/60">Total Consumed Today</p>
        <p class="font-heading font-black text-2xl text-brand-charcoal">
          {{ store.nutritionTotals.value.calories }} <span class="text-xs font-sans text-brand-charcoal/60 font-normal">/ {{ store.currentUser.value.target_calories }} kcal</span>
        </p>
      </div>

      <div class="flex items-center gap-3 text-right font-mono text-xs">
        <div>
          <span class="text-[10px] text-brand-charcoal/60 block">Protein</span>
          <span class="font-bold text-sky-700">{{ store.nutritionTotals.value.protein }}g</span>
        </div>
        <div>
          <span class="text-[10px] text-brand-charcoal/60 block">Carbs</span>
          <span class="font-bold text-amber-700">{{ store.nutritionTotals.value.carbs }}g</span>
        </div>
        <div>
          <span class="text-[10px] text-brand-charcoal/60 block">Fat</span>
          <span class="font-bold text-rose-700">{{ store.nutritionTotals.value.fat }}g</span>
        </div>
      </div>
    </div>
    ...
    <div class="space-y-6">
      ...
      >
        <div class="flex items-center justify-between px-1">
          <h3 class="font-heading font-bold text-sm uppercase text-brand-charcoal flex items-center gap-2">
            <span class="w-2 h-2 rounded-full bg-brand-accent"></span>
            {{ category }}
          </h3>
          <span class="text-[11px] font-mono text-brand-charcoal/60">
        ...
        <div v-if="getMealsByCategory(category).length > 0" class="space-y-3">
          <div
            ...
            :key="meal.id"
            class="bg-brand-gray border border-brand-charcoal/10 rounded-2xl p-4 space-y-3 hover:border-brand-charcoal/20 transition-colors shadow-sm"
          >
            <div class="flex gap-3">
              <img 
                :src="meal.image_url" 
                :alt="meal.meal_type" 
                class="w-16 h-16 rounded-xl object-cover border border-brand-charcoal/10 shrink-0"
              />
              <div class="flex-grow">
                <div class="flex items-center justify-between">
                  <h4 class="font-heading font-black text-sm text-brand-charcoal">
                ...
                <div class="flex gap-2 text-[10px] text-brand-charcoal/70 mt-1 font-mono">
                  <span class="text-sky-700 font-bold">P: {{ meal.total_protein }}g</span>
                  <span>•</span>
                  <span class="text-amber-700 font-bold">C: {{ meal.total_carbs }}g</span>
                  <span>•</span>
                  <span class="text-rose-700 font-bold">F: {{ meal.total_fat }}g</span>
                </div>
              </div>
            </div>
            ...
            <div class="flex flex-wrap gap-1.5 pt-1">
              <span
                ...
                :key="i"
                class="bg-brand-dark px-2 py-0.5 rounded-md text-[10px] text-brand-charcoal border border-brand-charcoal/10 font-medium"
              >
            ...
            <div 
              ...
              class="p-2.5 rounded-xl bg-brand-dark border border-brand-accent/30 text-xs space-y-1 shadow-sm"
            >
              <div class="flex items-center justify-between">
              ...
              <p v-if="meal.coach_comment" class="text-[11px] text-brand-charcoal/80 italic">
                "{{ meal.coach_comment }}"
              </p>
            </div>
          </div>
        ...
        <div 
          v-else
          class="p-4 rounded-xl border border-dashed border-brand-charcoal/15 bg-brand-gray/40 text-center text-xs text-brand-charcoal/60 flex items-center justify-between"
        >
