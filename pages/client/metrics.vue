...
<template>
  <div class="space-y-6">
    ...
    <div>
      ...
      <h2 class="font-heading font-black text-xl uppercase tracking-tight text-white">Metrics & Weigh-Ins</h2>
      <p class="text-xs text-gray-400 mt-0.5">Weekly scale weight, body fat %, and check-in history</p>
    </div>

    <!-- Quick Form -->
    <div class="bg-brand-gray border border-white/10 rounded-3xl p-5 space-y-4 shadow-xl">
      <h3 class="font-heading font-black text-sm uppercase text-white flex items-center gap-2">
        <Icon name="ph:scales-bold" class="w-4 h-4 text-brand-accent" />
        <span>Submit Weekly Check-In</span>
      </h3>

      <div v-if="successMessage" class="p-3 bg-emerald-500/20 border border-emerald-500 rounded-xl text-xs text-emerald-300 flex items-center gap-2">
      ...
      <div class="grid grid-cols-2 gap-3">
        <div class="space-y-1">
          <label class="text-[10px] uppercase font-bold text-gray-400">Scale Weight (kg) *</label>
          <input 
        ...
        <div class="space-y-1">
          <label class="text-[10px] uppercase font-bold text-gray-400">Body Fat % (optional)</label>
          <input 
      ...
      <div class="space-y-1">
        <label class="text-[10px] uppercase font-bold text-gray-400">Coach Notes & Energy Check</label>
        <input 
          v-model="notesInput"
          type="text"
      ...
      <button
        ...
        class="w-full bg-brand-accent text-black font-heading font-black text-xs uppercase tracking-wider py-3 rounded-xl hover:bg-white transition-all shadow-[0_0_15px_rgba(204,255,0,0.3)] disabled:opacity-40 disabled:cursor-not-allowed"
      >
        Log Check-In
      </button>
    </div>
    ...
    <div class="bg-brand-gray border border-white/10 rounded-3xl p-5 space-y-3">
      <div class="flex items-center justify-between">
        <h3 class="font-heading font-bold text-sm uppercase text-white">Weigh-In Trend History</h3>
      ...
      <div class="space-y-2">
        <div 
          ...
          :key="entry.id"
          class="bg-brand-dark/80 border border-white/5 rounded-xl p-3 flex items-center justify-between text-xs"
        >
          <div>
            <p class="font-mono font-bold text-white text-sm">
              {{ entry.weight_kg }} kg
              <span v-if="entry.body_fat_percentage" class="text-xs font-normal text-sky-400 ml-1">
                ({{ entry.body_fat_percentage }}% BF)
              </span>
            </p>
            <p v-if="entry.notes" class="text-[11px] text-gray-400 mt-0.5">{{ entry.notes }}</p>
          </div>
